...
if (isset($_POST['submit'])) {
    ...
    <?php if ($upload_files) { ?>
        <!----- uploader div start ------->
        <div class="uploader">
            <form action="dialog.php" method="post" enctype="multipart/form-data" id="myAwesomeDropzone"
                  class="dropzone">
            ...
            <center>
                <button class="btn btn-large btn-inverse close-uploader"><i
                            class="icon-backward icon-white"></i> <?php echo lang_Return_Files_List ?></button>
            </center>
    ...
    <div class="container-fluid">
                ...
                <div class="span4">
                    <?php if ($upload_files) { ?>
                        <button class="btn btn-inverse upload-btn" style="margin-left:5px;"><i
                                    class="icon-upload icon-white"></i> <?php echo lang_Upload_file ?></button>
                    <?php } ?>
                    <?php if ($create_folder) { ?>
                        <button class="btn new-folder" style="margin-left:5px;"><i
                                    class="icon-folder-open"></i> <?php echo lang_New_Folder ?></button>
                    <?php } ?>
                </div>
                    ...
                    <?php if ($_GET['type'] == 2 || $_GET['type'] == 0) { ?>
                        <div class="pull-right"><?php echo lang_Filter; ?> :
                            ...
                            <label id="ff-item-type-all" for="select-type-all"
                                   class="btn btn-inverse ff-label-type-all"><?php echo lang_All; ?></label>
        ...
        <div class="row-fluid">
            ...
            <ul class="breadcrumb">
                ...
                ?>
                <li class="pull-right"><a id="refresh"
                                          href="dialog.php?type=<?php echo $_GET['type'] ?>&editor=<?php echo $_GET['editor'] ? $_GET['editor'] : 'mce_0'; ?>&subfolder=<?php echo $subfolder ?>&popup=<?php echo $popup; ?>&field_id=<?php echo $_GET['field_id'] ? $_GET['field_id'] : ''; ?>&lang=<?php echo $_GET['lang'] ? $_GET['lang'] : 'en_EN'; ?>&fldr=<?php echo $subdir ?>&<?php echo uniqid() ?>"><i
            ...
            <div class="span12">
                ...
                <ul class="grid cs-style-2">
                                        ...
                                        <div class="img-precontainer">
                                            <div class="img-container directory"><span></span>
                                                <img class="directory-img"
                                                     src="ico/folder<?php if ($file == '..') echo "_return" ?>.png"
                                                     alt="folder"/>
                                            </div>
                                        </div>
                                    </a>
                                    ...
                                    <?php } else { ?>
                                        ...
                                        <figcaption>
                                            <a href="javascript:void('')"
                                               class="erase-button"
                                               <?php if ($delete_folder){ ?>onclick="if(confirm('<?php echo lang_Confirm_Folder_del; ?>')){ delete_folder('<?php echo $root . $cur_dir . $file; ?>','<?php echo $thumbs_path . $subdir . $file; ?>'); $(this).parent().parent().parent().hide(200); return false;}"<?php } ?>
                                               title="<?php echo lang_Erase ?>">
                    ...
                    <li class="ff-item-type-<?php echo $class_ext; ?>">
                        <figure>
                            <a href="javascript:void('');" title="<?php echo lang_Select ?>"
                               onclick="<?php echo $apply . "('" . $file . "'," . $_GET['type'] . ",'" . $_GET['field_id'] . "');"; ?>">
                                <div class="img-precontainer">
                                    <div class="img-container"><span></span>
                                        <img data-src="holder.js/122x91"
                                             alt="image" <?php echo $show_original ? "class='original'" : "" ?>
                                             src="<?php echo $src_thumb; ?>">
                                    </div>
                                </div>
                            </a>
                            <div class="box">
                            ...
                            <figcaption>
                                <form action="force_download.php" method="post" class="download-form"
                                    ...
                                    <a title="<?php echo lang_Download ?>" class="" href="javascript:void('');"
                                       onclick="$('#form<?php echo $nu; ?>').submit();"><i
                                                class="icon-download"></i></a>
                                    <?php if ($is_img) { ?>
                                        <a class="preview" title="<?php echo lang_Preview ?>"
                                    ...
                                    <a href="javascript:void('');" class="erase-button"
                                       <?php if ($delete_file){ ?>onclick=" if(confirm('<?php echo lang_Confirm_del; ?>')){ delete_file('<?php echo $root . $cur_dir . $file; ?>','<?php echo $thumbs_path . $subdir . $file; ?>'); $(this).parent().parent().parent().parent().hide(200); return false;}"<?php } ?>
                                       title="<?php echo lang_Erase ?>"><i
